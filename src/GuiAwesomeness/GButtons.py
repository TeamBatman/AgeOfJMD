...
class GSmallButton(GButton):
    def __init__(self, parent, text="", command=None, color=0, iconPath=None):
        super(GSmallButton, self).__init__(parent, text, command, color)
        self.imageData['ICON'] = Image.open(iconPath) if iconPath else Image.new('RGBA', (70, 70))
        self.imageData['ICON'].thumbnail((30, 30), Image.ANTIALIAS)
        self.icon = ImageTk.PhotoImage(self.imageData['ICON'])

    def _determineColor(self, color):
        self.imageData[GButton.NORMAL] = Image.open("GuiAwesomeness/Gui/Buttons/buttonSquare_brown.png")
        self.imageData[GButton.PRESSED] = Image.open("GuiAwesomeness/Gui/Buttons/buttonSquare_brown_pressed.png")
        self.imageData[GButton.FOCUS] = Image.open('GuiAwesomeness/Gui/Buttons/buttonSquare_focus.png')
    ...
    def draw(self, x, y):
        ...
        check_x = x + self.width / 2 - width / 2
        check_y = y + self.height / 2 - height / 2
        self.parent.create_image(check_x, check_y, anchor=NW, image=self.icon, tags=self.id)
